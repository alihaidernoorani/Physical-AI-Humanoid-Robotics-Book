...
<Exercise
  ...
  problem="Create a lifecycle node in Python that manages a humanoid robot's sensor driver with proper state transitions."
  hints={[
...
<Quiz
  ...
  correctAnswer="To represent a configured node ready to operate but not currently active"
