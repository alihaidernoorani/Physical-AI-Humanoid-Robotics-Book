...
<Exercise
  ...
  hints={[
    "Use the Isaac Sim scene setup tools",
    "Configure appropriate lighting and furniture",
    "Set up the camera and sensor parameters"
  ]}
...
```mermaid
graph TD
    DR[Domain Randomization] --> L[Lighting]
    DR --> T[Textures]
    DR --> O[Objects]
    DR --> E[Environment]
    L --> LP[Position/Intensity/Color]
    T --> TM[Materials/Patterns]
    O --> OP[Placement/Orientation]
    E --> EP[Weather/Atmosphere]
```
...
```mermaid
graph LR
    SC[Scene] --> RGB[RGB Camera]
    SC --> DS[Depth Sensor]
    SC --> LD[LiDAR]
    RGB --> SYNC[Synchronization]
    DS --> SYNC
    LD --> SYNC
    SYNC --> FD[Fused Dataset]
```
...
```mermaid
graph LR
    I[Input Image] --> SS[Semantic Segmentation]
    SS --> F[Floor Class]
    SS --> W[Wall Class]
    SS --> FU[Furniture Class]
    SS --> OB[Obstacle Class]
    F & W & FU & OB --> LI[Labeled Image]
```
