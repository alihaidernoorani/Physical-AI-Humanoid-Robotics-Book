...
```mermaid
graph LR
    C[Camera Input] --> FE[Feature Extraction]
    FE --> FT[Feature Tracking]
    FT --> PE[Pose Estimation]
    PE --> M[Mapping]
    M --> LC[Loop Closure]
...
```mermaid
graph LR
    I[Input Image] --> PP[Preprocessing]
    PP --> DNN[DNN Inference]
    DNN --> D2D[2D Detection]
    D2D --> CL[Classification]
    CL --> D3D[3D Extension]
    D3D --> OUT[Object Detections]
```
...
```mermaid
graph TD
    M[Model] --> TRT[TensorRT Optimization]
    TRT --> MEM[Memory Management]
    MEM --> PIPE[Pipeline Optimization]
    PIPE --> SCHED[Resource Scheduling]
    SCHED --> RT[Real-time Inference]
```
