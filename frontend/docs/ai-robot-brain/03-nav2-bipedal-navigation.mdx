...
```mermaid
graph LR
    G[Goal] --> GP[Global Planner]
    GP --> LP[Local Planner]
    LP --> BC[Balance Constraints]
    BC --> FP[Footstep Planner]
    FP --> EX[Execution]
```
...
```mermaid
graph TD
    IN[Isaac Navigation] --> SLAM[GPU SLAM]
    IN --> PP[Path Planning]
    IN --> OD[Obstacle Detection]
    SLAM --> N2[Nav2 Stack]
    PP --> N2
    OD --> N2
    N2 --> EX[Execution]
...
```mermaid
graph LR
    GP[Global Path] --> ZMP[ZMP Validation]
    ZMP --> FS[Footstep Sequence]
    FS --> SM[Stability Margins]
    SM --> WP[Walking Pattern]
    WP --> EX[Execution]
```
