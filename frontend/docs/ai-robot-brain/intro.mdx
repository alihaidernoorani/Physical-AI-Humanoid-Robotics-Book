...
graph LR
    ...
    PL <--> M
```
*Figure: Diagram showing the interconnected systems of perception, planning, and control in the AI-robot brain architecture*
...
```mermaid
graph LR
    CAM[Camera] --> DNN[DNN Inference]
    LID[LiDAR] --> PC[Point Cloud]
    DNN --> OD[Object Detection]
    PC --> SLAM[VSLAM]
    OD --> WM[World Model]
    SLAM --> WM
```
*Figure: Diagram showing the GPU-accelerated perception pipeline in Isaac ROS GEMs*
...
```mermaid
graph LR
    G[Goal] --> PP[Path Planner]
    PP --> SP[Step Planner]
    SP --> BC[Balance Control]
    BC --> LL[Leg Controller]
    LL --> R[Robot Motion]
```
*Figure: Diagram showing bipedal navigation planning with step location and balance constraints for humanoid robots*
