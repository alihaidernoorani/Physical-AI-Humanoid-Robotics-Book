...
```mermaid
graph LR
    P[Publisher] --> R{Reliability}
    R -->|Reliable| G[Guaranteed Delivery]
    R -->|Best Effort| F[Fast but May Lose]
    P --> D{Durability}
    D -->|Volatile| N[No Late-Join Data]
    D -->|Transient| L[Late-Join Gets Latest]
```
...
```mermaid
graph LR
    C[Action Client] -->|1. Send Goal| S[Action Server]
    S -->|2. Accept/Reject| C
    S -->|3. Feedback Loop| C
    S -->|4. Final Result| C
```
