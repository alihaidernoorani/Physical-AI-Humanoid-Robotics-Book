...
```mermaid
graph LR
    NL[Natural Language] --> SP[Semantic Parser]
    SP --> SR[Structured Representation]
    SR --> CAP[Robot Capabilities]
    SR --> ENV[Environment Objects]
    CAP --> EX[Executable Commands]
    ENV --> EX
```
...
```mermaid
graph TD
    HL[High-Level Goal] --> ML1[Mid-Level Task 1]
    HL --> ML2[Mid-Level Task 2]
    ML1 --> LL1[Low-Level Action 1]
    ML1 --> LL2[Low-Level Action 2]
    ML2 --> LL3[Low-Level Action 3]
    ML2 --> LL4[Low-Level Action 4]
```
...
```mermaid
graph LR
    LR[Linguistic Reference] --> SC[Spatial Context]
    ENV[Environment Perception] --> SC
    SC --> GL[Geometric Location]
    GL --> RS[Robot Coordinate System]
```
...
```mermaid
graph TD
    CMD[Ambiguous Command] --> AN[Analysis]
    AN --> CR[Clarification Request]
    CR --> UR[User Response]
    UR --> RS[Resolved Command]
    RS --> EX[Execution]
```
