...
```mermaid
graph LR
    NL[Natural Language] --> SM[Semantic Mapping]
    SM --> RC[Robot Capabilities]
    SM --> EO[Environmental Objects]
    RC --> AS[Action Selection]
    EO --> AS
    AS --> ROS[ROS 2 Actions]
```
...
```mermaid
graph TD
    HL[High-Level Action Server] --> ML1[Navigation Server]
    HL --> ML2[Manipulation Server]
    HL --> ML3[Perception Server]
    ML1 --> LL1[Path Planning]
    ML1 --> LL2[Motion Control]
    ML2 --> LL3[Grasp Planning]
    ML2 --> LL4[Arm Control]
```
...
```mermaid
graph LR
    RS[Robot State] --> FB[Feedback System]
    FB --> VIS[Visual Display]
    FB --> AUD[Audio Speech]
    FB --> HAP[Haptic Signals]
    VIS --> USR[User]
    AUD --> USR
    HAP --> USR
```
...
```mermaid
graph LR
    LD[Linguistic Description] --> VG[Visual Grounding]
    VO[Visual Observations] --> VG
    VG --> OI[Object Identification]
    OI --> LOC[Location]
    OI --> ATTR[Attributes]
```
...
```mermaid
graph LR
    VTT[Voice-to-Text] --> LLM[LLM Processing]
    LLM --> LAG[Language-Action Grounding]
    LAG --> ROS[ROS 2 Actions]
    ROS --> NAV[Navigation]
    ROS --> MAN[Manipulation]
    ROS --> PER[Perception]
```
